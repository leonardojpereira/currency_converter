...
function main() {
    let currency = { "BRL": "Real", "EUR": "Euro", "USD": "Dólar" };
    let options = [];
  ...
  async function calculate() {
    ...
    let rate = rates[to];
    tax_info.innerHTML = `Cotação atualizada: <strong>1 ${getInfoSelect(label_from_currency)}</strong> = <strong>${rate} ${getInfoSelect(label_to_currency)}</strong>`
